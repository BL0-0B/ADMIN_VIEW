...
function posSearch(query) {
  ...
  // Auto-detect barcode scan (8+ chars) — show qty input bar
  if (query.length >= 8) {
    const match = store.products.find(p => p.barcode === query.trim());
    if (match) {
      // Show qty bar
      pendingScanProduct = match;
      const bar = document.getElementById('scan-qty-bar');
      const nameEl = document.getElementById('scan-qty-product-name');
      const qtyEl  = document.getElementById('scan-qty-input');
      if (bar) bar.style.display = 'flex';
      if (nameEl) nameEl.textContent = match.name + ` — ₱${match.price.toFixed(2)} (${match.stock} in stock)`;
      if (qtyEl) { qtyEl.value = 1; qtyEl.max = match.stock; setTimeout(() => qtyEl.focus(), 80); }
